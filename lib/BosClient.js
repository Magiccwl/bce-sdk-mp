...
export default class BosClient extends EventBus {
    ...
    sendRequest(httpMethod, varArgs) {
        let args = Object.assign({}, defaultRequestArgs, varArgs);
    ...
    _prepareObjectHeaders(options) {
        let headers = pickBy(options, (value, key) => (allowedHeaders.indexOf(key) > -1) || String.prototype.startsWith.call(key, 'x-bce-meta-'));
        let metaSize = Object.entries(headers).reduce((ret, [key, value]) => {
